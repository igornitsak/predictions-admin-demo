...
| Add markets | Multiple checkbox | Match Result, Correct Score, Total Goals, etc. |
...
- Если выбрана опция "Simple Contest", то заданые Points & Ranges на уровне настроек Project уже не действуют для этого турнира. Также нельзя задать Odds range, поскольку в этом нет смысла.
...
Таблица со списком пользователей-участников турнира и колонки:
...
Пользователи получают points за один tip согласно правилам, заданным в настройках проекта либо на уровне сущности Contest (опция Simple Contest). За tips со статусом Lost, Push или Void пользователь получает 0 points.

Расчёт Points для одного tip (пример) по заданым настройкам на уровне проекта:
...
ВАЖНО! Если в рамках Contest активирована опция "Simple Contest", то для этого турнира автоматически активируются только 2 маркета: 1X2, Correct Score. За правильно угаданый результат по кажному из этих маркетов можно указать точное кол-во очков. Опция "Simple Contest" отменяет градацию очков с привязкой к вероятностям, которая задается на уровне проекта. Также для "Simple Contest" не нужно задавать минимальный и максимальний коэффициент. 
...
- Если tip проигрывает наполовину, сумма вычитается из общего результата (-100 coins x Odds — например, -100 X 1.45 = -145).
- Если tip имеет статус Void или Push, он стоит 0 coins (0 coins x Odds — например, 0 X 1.45 = 0).
